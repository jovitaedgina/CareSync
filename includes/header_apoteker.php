...
<body class="bg-gray-50 text-gray-800 font-sans antialiased flex h-screen overflow-hidden">
    ...
    <aside class="w-64 bg-white shadow-lg border-r border-gray-200 flex flex-col hidden md:flex z-20">
        ...
        <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
            <a href="dashboard.php" class="flex items-center px-4 py-3 rounded-xl transition <?= $current_page === 'dashboard.php' ? 'bg-teal-600 text-white shadow-md' : 'text-gray-600 hover:bg-teal-50 hover:text-teal-600' ?>">
                <i class="fa-solid fa-clipboard-list w-6"></i>
                <span class="font-medium ml-3">Antrean Resep</span>
            </a>
            <a href="inventory.php" class="flex items-center px-4 py-3 rounded-xl transition <?= $current_page === 'inventory.php' ? 'bg-teal-600 text-white shadow-md' : 'text-gray-600 hover:bg-teal-50 hover:text-teal-600' ?>">
                <i class="fa-solid fa-boxes-stacked w-6"></i>
                <span class="font-medium ml-3">Stok Obat</span>
            </a>
            <a href="history.php" class="flex items-center px-4 py-3 rounded-xl transition <?= $current_page === 'history.php' ? 'bg-teal-600 text-white shadow-md' : 'text-gray-600 hover:bg-teal-50 hover:text-teal-600' ?>">
            ...
            <div class="flex items-center space-x-4 ml-auto">
                ...
                <div class="relative ml-2 pl-4 border-l border-gray-200" x-data="{ profileOpen: false }">
                    <button @click="profileOpen = !profileOpen" @click.away="profileOpen = false" class="flex items-center gap-3 focus:outline-none hover:bg-gray-50 py-1 px-2 rounded-lg transition">
                        <div class="w-8 h-8 rounded-full bg-teal-100 text-teal-700 flex items-center justify-center font-bold border border-teal-200"><?= htmlspecialchars($staffProfile['initials']) ?></div>
                        <div class="hidden sm:flex flex-col text-left">
                            <span class="text-sm font-bold text-gray-800 leading-none"><?= htmlspecialchars($staffProfile['name']) ?></span>
                            <span class="text-[10px] text-gray-500 mt-1 leading-none">Apoteker</span>
                        </div>
